...
1. **Real stack is heavy:** Python + MuJoCo + drivers + calibration takes time to install. Not everyone has it, and you can't embed `mujoco.viewer` in a blog post, docs site, workshop slides, chat UI, or agent UI.
2. **Zero-install learning:** Drag a target, move joints, see telemetry, record/replay, and understand observation → policy → action → step → recording in 30 seconds.
3. **Embeddable:** Works anywhere a browser works — docs, LMS, Hatch chat, other agents, workshops. That's the whole point of putting "browser" in the name.
4. **Teaching the stack:** Includes a responsibility map — what MuJoCo does, what LeRobot does, what your env does, what ROS 2 does later — which upstream viewers don't explain.
...
Keep the playgrounds as educational wrappers, clearly credited, and focus novelty claims on the infrastructure layer.
